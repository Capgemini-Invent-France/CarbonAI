...
class NvidiaPower(GpuPower):
    ...
    def parse_power_log(self):
        ...
        if not self.log_file.exists():
            raise FileNotFoundError("Logging file not found, make sure you started to run a mesure")
        ...
        self.recorded_power = {
            TOTAL_TIME: df["Elapsed time"].iloc[-1],
            TOTAL_ENERGY_GPU:df["Power"].sum()*self.interval*1000/3600
        }
